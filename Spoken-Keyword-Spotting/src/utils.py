...
def build_feature_extractor(model, layer_name):
    """
    Rebuild a functional feature extractor up to layer_name, sharing the loaded
    weights. Keras 3 no longer exposes `.input` on a Sequential loaded from .h5,
    so we re-thread the loaded layers onto a fresh Input.
    :param model: Loaded Sequential model
    :param layer_name: Name of the layer whose output becomes the embedding
    :return: Functional Model mapping input -> layer_name output
    """

    feat_input = tf.keras.Input(shape=INPUT_SHAPE)
    x = feat_input
    ...
    return tf.keras.Model(inputs=feat_input, outputs=x)
...
def getDataset(df, batch_size, cache_file=None, shuffle=True, parse_param=PARSE_PARAMS, scale=False):
    ...
    # py_function erases static shape; Keras 3 requires a known rank. Restore it.
    def _set_shapes(fbank, label):
        fbank.set_shape(INPUT_SHAPE)
        label.set_shape([])
        return fbank, label
