...
from utils import getDataset, plot_history, build_feature_extractor
...
def marvin_kws_model():
    ...
    # Apply PCA to reduce dimensionality (whiten -> unit-variance components so
    # the One-Class SVM's RBF kernel stays numerically well-conditioned)
    ...
    def svm_objective(**params):
        ...
        # Some gamma/nu combinations make libSVM produce non-finite dual
        # coefficients (sklearn >=1.x raises instead of returning garbage).
        # Penalize those so the optimizer steers away from them.
        try:
