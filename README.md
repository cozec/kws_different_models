...
|---|------|----------|-------|---------|--------|
| 1 | [Spoken-Keyword-Spotting](Spoken-Keyword-Spotting/) | CNN embeddings → PCA → One-Class SVM ("marvin" hotword) | — | Google Speech Commands v0.01 | 99.6% acc, F1 0.88 (true test set) |
| 2 | [KWS_pytorch](KWS_pytorch/) | Small CNN on STFT + **filler class** | Deep KWS (Chen, Parada & Heigold, ICASSP 2014) | MobvoiHotwords (OpenSLR-87, ~6% subset) | 96.3% acc; FA 0.073% @ FR 19.3% (τ=0.9) |
| 3 | [Wav2Keyword](Wav2Keyword/) | Wav2Vec 2.0 base fine-tune + conv head | Wav2KWS (Seo et al., IEEE Access 2021) | Google Speech Commands v0.01, 22 classes | **97.23% in 3 epochs** (paper: 97.9% @ 100 epochs) + live streaming demo |

Candidate survey that led to these picks: [KWS_GITHUB_PROJECTS.md](KWS_GITHUB_PROJECTS.md).

---

## 1. Spoken-Keyword-Spotting (vineeths96)
...
## 2. KWS_pytorch (hongfeixue) + Deep KWS filler class
...
## 3. Wav2Keyword (qute012 / dobby-seo) — Wav2KWS
...
- macOS / Apple Silicon (arm64), Python 3.12 (brew) — system Python 3.14 has
  no TF/torch wheels
- Repo 1: own `.venv` — TensorFlow 2.21, Keras 3.15, scikit-learn 1.9,
  scikit-optimize, python_speech_features
- Repos 2–3: shared `.venv` in `KWS_pytorch/` — PyTorch 2.13 (MPS), torchaudio
  2.11, soundfile, sounddevice
